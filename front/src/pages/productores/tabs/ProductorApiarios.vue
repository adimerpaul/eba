<template>
  <q-card-section>
    <div class="row items-center q-gutter-sm q-mb-sm">
      ...
      <q-space />
      <q-btn dense color="primary" icon="add_circle" label="Nuevo" no-caps @click="startCreate" :disable="saving" />
    </div>
    ...
    <q-markup-table dense>
      ...
      <tbody>
        <tr v-for="(a, i) in list" :key="a.id">
          ...
          <td class="text-center">
            <q-chip :color="a.cosechas_total > 0 ? 'green' : 'grey'" text-color="white" size="sm">
              {{ a.cosechas_total || 0 }}
            </q-chip>
          </td>
          <td class="text-center">
      ...
      <q-card style="min-width: 900px; max-width: 95vw;">
        ...
        <q-card-section>
          <q-form @submit="saveApiario" class="row q-col-gutter-sm">
            ...
            <!-- MAPA -->
            <div class="col-12">
              <ApiarioMap ref="apiarioMap" v-model="formApiario" :center="[-16.5, -68.15]" :zoom-init="14" />
            </div>

            <div class="col-6 col-sm-3">
              <q-input v-model.number="formApiario.numero_colmenas_runsa" type="number" label="Colmenas RUNSA" dense
                outlined />
            </div>
            <div class="col-6 col-sm-3">
              <q-input v-model.number="formApiario.numero_colmenas_prod" type="number" label="Colmenas PROD." dense
                outlined />
            </div>
            <div class="col-6 col-sm-3">
              <q-input v-model.number="formApiario.rend_programa_nal" type="number" step="0.01" label="Rend. prog. nal"
                dense outlined />
            </div>
            <div class="col-6 col-sm-3">
            ...
            <div class="col-12 text-right q-gutter-sm q-mt-sm">
              ...
              <q-btn color="primary" :label="formApiario.id ? 'Guardar cambios' : 'Crear'" type="submit"
                :loading="saving" />
            </div>
          </q-form>
        </q-card-section>
      ...
      <q-card style="min-width: 900px; max-width: 95vw;">
        ...
        <q-card-section>
          <q-markup-table dense>
            ...
            <tbody>
              <tr v-for="(c, i) in colmenas" :key="c.id || i">
                ...
                <td><q-chip :color="c.estado === 'ACTIVA' ? 'green' : 'grey'" text-color="white" size="xs">{{ c.estado
                    }}</q-chip></td>
              </tr>
          ...
          <q-form @submit="saveColmena" class="row q-col-gutter-sm">
            <div class="col-12 col-sm-3"><q-input v-model="formColmena.codigo_colmena" dense outlined label="Código" />
            </div>
            <div class="col-12 col-sm-3"><q-input v-model="formColmena.tipo_colmena" dense outlined
                label="Tipo colmena" /></div>
            <div class="col-12 col-sm-3">
              <q-select v-model="formColmena.tipo_miel_id" :options="tipoMielOptions" option-value="id"
                option-label="tipo_miel" emit-value map-options dense outlined label="Tipo de miel" />
            </div>
            <div class="col-12 col-sm-3"><q-input v-model="formColmena.fecha_instalacion" type="date" dense outlined
                label="Instalación" /></div>
            <div class="col-12 col-sm-4"><q-input v-model="formColmena.reina_fecha_nacimiento" type="date" dense
                outlined label="Reina: nacimiento" /></div>
            <div class="col-12 col-sm-4"><q-input v-model="formColmena.reina_procedencia" dense outlined
                label="Reina: procedencia" /></div>
            <div class="col-12 col-sm-4"><q-select v-model="formColmena.estado" :options="['ACTIVA', 'INACTIVA']" dense
                outlined label="Estado" /></div>
            <div class="col-12 text-right q-gutter-sm">
              <q-btn flat color="grey" label="Limpiar" @click="resetColmena" :disable="saving" />
              <q-btn color="primary" :label="formColmena.id ? 'Guardar cambios' : 'Agregar colmena'" type="submit"
                :loading="saving" />
            </div>
          </q-form>
        </q-card-section>
