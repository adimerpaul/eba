...
<!--            <div class="ellipsis-2-lines" v-html="preview(row.html)"></div>-->
          ...
          <td>{{ row.user?.name || 'ÔÇö' }}</td>
        </tr>
        </tbody>
      </q-markup-table>
    </q-card-section>

    <!-- Di├ílogo Crear/Editar -->
          ...
          <div class="row q-col-gutter-md">
            ...
            <div class="col-12">
              ...
              <q-editor
                v-model="form.html"
                min-height="320px"
                placeholder="Escriba aqu├¡ el contenido de la sesi├│n..."
                :toolbar="[
...
export default {
  name: 'DocumentosOriginal',
  props: {
    cosecha: {type: Object, required: true},
    // opcional: si ya tienes el id de usuario en tu layout, puedes pasarlo aqu├¡
  ...
  data() {
    return {
      ...
      planillas: [
        // ACTA DE RECEPCI├ôN Y CONFORMIDAD DE MATERIA PRIMA
        ...
        {
          id: 1,
          nombre: 'Acta de Recepci├│n y Conformidad de Materia Prima',
          html: '<h1>Acta de Recepci├│n y Conformidad de Materia Prima</h1><p>Contenido del acta...</p>'
        },
        {
          id: 2,
        ...
        {
          id: 5,
          nombre: 'Recepci├│n de Materia Prima',
          html: '<h1>Recepci├│n de Materia Prima</h1><p>Contenido del documento...</p>'
        },
        {
          id: 6,
  ...
  methods: {
    aplicarPlantilla(plantillaId) {
      ...
      if (plantilla) {
        this.form.html = ''
        if (plantilla.nombre === 'Acta de Recepci├│n y Conformidad de Materia Prima' ){
    ...
    //   } catch (e) {
    //     // silencioso: si falla, igual te dejar├í guardar si seteas user_id manualmente
    //   }
    // },
    // preview(html) {
    //   if (!html) return ''
    //   const text = html.replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim()
    //   return text.length > 140 ? text.slice(0, 140) + 'ÔÇª' : text
    // },
    resetForm() {
      this.form = {
